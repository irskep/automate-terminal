...
def test_vscode_terminal_detect(
    variant, term_program, cursor_trace_id, expected, fake_applescript, monkeypatch
):
    ...
    terminal = VSCodeTerminal(fake_applescript, variant=variant)
    assert terminal.detect(term_program, "Darwin") == expected


def test_vscode_terminal_properties(fake_applescript):
    """Test that VSCode terminal properties vary by variant."""
    vscode = VSCodeTerminal(fake_applescript, variant="vscode")
    ...
    cursor = VSCodeTerminal(fake_applescript, variant="cursor")
    assert cursor.cli_command == "cursor"
    assert cursor.display_name == "Cursor"
    assert "Cursor" in cursor.app_names


def test_vscode_terminal_capabilities(fake_applescript):
    """Test that VSCode terminal reports correct capabilities."""
    terminal = VSCodeTerminal(fake_applescript, variant="vscode")
    caps = terminal.get_capabilities()

    assert caps.can_create_tabs is False
    assert caps.can_create_windows is True
    assert caps.can_list_sessions is False
    assert caps.can_switch_to_session is True  # macOS via AppleScript
